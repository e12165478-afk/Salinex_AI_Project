...
def main():
    ...
    # ISSUE 8:  Watermark
    # ========================================================================
    print_section("🔍 ISSUE 8:  Watermark in UI")

    _patterns = {
        '': r'||detect.*ai|ai.*detect',
    }

    for py_file in py_files:
        if 'salinex_env' in str(py_file):
            continue
        matches = scan_file_for_patterns(py_file, _patterns)
        for m in matches:
            print_issue(8, "WARNING", " watermark found in production UI code",
                       file_path=py_file, line_num=m['line'],
                       code_snippet=m['text'],
    ...
    actions = [
        ...
        ("WARNING", "All .py files", "Remove '' watermarks"),
    ]
