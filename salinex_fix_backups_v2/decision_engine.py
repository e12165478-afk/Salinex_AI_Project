...
def generate_prescriptive_advice(li_ppm, mg_ppm, ca_ppm, elec_price, current_flow, profit, roi_years):
    ...
    # ۲. تحلیل نرخ بازگشت سرمایه (ROI)
    if roi_years > 5.0:
        suggested_flow = round(current_flow * (roi_years / 5.0), 0)
        advice_list.append(
            f"⚠️ **بهینه‌سازی ROI:** نرخ بازگشت سرمایه فعلی ({round(roi_years,1)} سال) بالاتر از تارگت ۵ ساله است. "
            f"توصیه هوشمند: دبی ورودی (Flow Rate) را به حدود **{suggested_flow} m3/hr** افزایش دهید تا ROI زیر ۵ سال بیاید."
        )
    else:
        advice_list.append(
