...
st.set_page_config(page_title="Salinex AI - Full Integrated Suite v3.9.5", layout="wide")
PROJECT_DIR = r"Path(__file__).resolve().parent.parent"
...
st.markdown("**ENGINEER:** Mirzaee | **PHASE:** Closing Week 3 (Ensemble ML & Adaptive Engine)")
...
if uploaded_file is not None and df is not None:
    try:
        ...
        payback_years = (capex_val * 1000000) / (profit * 350)  # FIXED: Was incorrectly named payback_years if profit > 0 else float('inf')
        ...
        for item in advices:
            ...
            if "inf" in str(item).lower():
                item = "⚠️ **بهینه‌سازی بازگشت سرمایه:** فرآیند در شرایط فعلی زیان‌ده است (سود روزانه منفی). جهت اقتصادی شدن و کاهش ROI، افزایش دبی عملیاتی یا بهینه‌سازی هزینه‌های برق و مواد پیش‌تصفیه توصیه می‌شود."
            st.info(item)
